...
select ok(public.is_supported_currency('JPY'), 'JPY is supported');
select ok(public.is_supported_currency('KWD'), 'KWD is supported');
...
select is(
  (public.prepare_catalog_import(
    ...
    '[{"sku":"CSV-JPY","description":"Zero-decimal currency","unit_code":"EA","quantity_precision":"0","unit_price":"1","currency_code":"JPY","tax_code":"IN_GST18","active":"true"}]'::jsonb
  ) ->> 'invalid_count')::integer,
  0,
  'CSV review accepts JPY'
);
select is(
  (select normalized_payload ->> 'currency_code' from public.catalog_import_rows where normalized_payload ->> 'sku' = 'CSV-JPY'),
  'JPY',
  'CSV review stores JPY as canonical currency'
);
reset role;
select is(public.parse_currency_minor('12.345', 'KWD'), 12345::bigint, 'KWD import parsing uses three decimals');
select is(public.parse_currency_minor('1234', 'JPY'), 1234::bigint, 'JPY import parsing uses zero decimals');
set local role authenticated;
set local request.jwt.claims = '{"sub":"11111111-1111-4111-8111-111111111111","role":"authenticated"}';
select is(
  (public.prepare_catalog_import(
